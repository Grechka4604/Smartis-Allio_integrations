...
## 6. Маппинг в CRM-схему для Броней
...
# Получение сделок (контрактов) из ALLIO
...
2. **Итерация по интеграциям**  
   Для каждой активной интеграции выполняется цикл синхронизации.
3. **Итерация по дням**  
   Синхронизация выполняется по дням в заданном диапазоне (например, за последние N дней).  
   Для каждой даты формируется отдельный запрос к API ALLIO.
4. **Вызов API ALLIO**  
   Для текущей интеграции и даты отправляется POST-запрос к эндпоинту:   POST /bugh1c/contracts/number 
...
6. **Повтор для всех дней и интеграций**  
Процесс повторяется до полной обработки всех дат и всех активных интеграций.

### Ограничения и рекомендации

- **Rate limiting**: запросы к API ALLIO должны учитывать лимит — не более 2 запросов в секунду на один IP адрес.
- **Идемпотентность**: повторная синхронизация одного и того же дня не должна создавать дубли.
- **Логирование**: все операции логируются для диагностики и аудита.
